...
from .serializers import  (
    ProgramSerializer, Profile, SubjectSerializer, EventSerializer, TeacherSerializer, FeedbackUserSerializer,
    TestimonialSerializer, ProfileSerializer, UserSerializer, StudentSerializer, GradeSerializer, ReviewSerializer
)

from .models import  Program, Subject, Event, Teacher, FeedbackUser, Testimonial, Student, Grade, Review
...
class SubjectViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
            feedback_user, created = User.objects.get_or_create(email = self.request.data.get("email"), defaults={"name":self.request.data.get("name")})
            serializer.save(user=feedback_user)  
